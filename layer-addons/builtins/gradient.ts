...
function gradientSampleExpression(binding: GradientLayerShaderBinding) {
  ...
  const branches = Array.from({ length: Math.max(0, binding.stopCount - 1) }, (_, index) => {
    ...
    return `${keyword} (gradientT <= ${nextStopT}) {
      let ${localT}: f32 = clamp((gradientT - ${currentStopT}) / max(${nextStopT} - ${currentStopT}, 0.00001), 0.0, 1.0);
  ...
  return `{
    let gradientAxis = normalize(${binding.parameterPrefix}Axis);
...
const gradientWebGpuAdapter: BuiltInWebGpuLayerAdapter<"gradient", GradientLayerShaderBinding, GradientUniformNodes> = {
  ...
  createSampleExpression: (layer, _language, context) => {
    ...
    return binding ? gradientSampleExpression(binding) : zeroEffectExpression();
  },
