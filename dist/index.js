import { $ as e, A as t, D as n, F as r, G as i, H as a, I as o, J as s, K as c, L as l, M as u, N as d, O as f, P as p, Q as m, R as h, U as g, V as _, W as ee, X as te, Y as v, Z as y, d as b, et as ne, it as re, j as ie, k as ae, m as oe, nt as se, q as x, rt as S, t as C, tt as ce, z as w } from "./starfield-bake-registry-BWrHxpvc.js";
import * as T from "three";
import { NodeMaterial as E } from "three/webgpu";
import { Fn as D, cameraProjectionMatrixInverse as le, cameraWorldMatrix as ue, modelViewProjection as O, normalize as de, positionGeometry as fe, screenUV as pe, texture as k, uniform as A, vec2 as me, vec4 as he, wgslFn as j } from "three/tsl";
...
function _e(e = M) {
	return ge(e).type === "sphere" ? new T.SphereGeometry(1, 64, 32) : new T.BoxGeometry(1, 1, 1);
}
function ve(e = 1, t = 25, n = 25) {
	let r = [], i = (t, n) => {
...
function Ce(e, t) {
	let n = Math.floor(e), r = Math.floor(t), i = e - n, a = t - r, o = i * i * (3 - 2 * i), s = a * a * (3 - 2 * a), c = B(n, r), l = B(n + 1, r), u = B(n, r + 1), d = B(n + 1, r + 1), f = c + (l - c) * o;
	return f + (u + (d - u) * o - f) * s;
}
function we(e, t) {
	let n = e, r = t, i = .5, a = 0;
	for (let e = 0; e < be; e += 1) a += i * Ce(n, r), n *= 2, r *= 2, i *= .5;
	return a;
}
function Te(e, t) {
	let n = Se(t), [i, o, s] = r(e);
	...
	let c = 1 + -.9 * n.elevation, l = i / (o * c) * n.scale + n.offset, u = s / (o * c) * n.scale + n.offset, d = xe.reduce((e, t) => e + we(l * t.scale + t.offset, u * t.scale + t.offset) * t.weight, 0) * .5 + .5, f = 1 - n.coverage, p = y(a(f, f + .3, d) * a(0, .1 + .2 * n.elevation, o) * n.density);
	if (p <= 1e-5) return [
		0,
		0,
		0,
...
function Ee(e) {
	return e.map((e) => {
		let t = Se(e.layer.params);
		return {
			color: A(t.color),
			coverage: A(t.coverage),
			density: A(t.density),
			elevation: A(t.elevation),
			layerId: e.layer.id,
			offset: A(t.offset),
			scale: A(t.scale),
			shadowColor: A(t.shadowColor),
			sunDirection: A(t.sunDirection)
		};
	});
}
function De(e, t) {
...
function Ae(e) {
    ...
    if (cloudDirection.y > 0.0 && ${t}Coverage > 0.0) {
      ...
      ${xe.map((e, t) => `
      {
        ...
        var sum${t}: f32 = 0.0;
        for (var octave${t}: i32 = 0; octave${t} < ${be}; octave${t} = octave${t} + 1) {
          let cell${t} = floor(p${t});
          let frac${t} = p${t} - cell${t};
          let weight${t} = frac${t} * frac${t} * (3.0 - 2.0 * frac${t});
          ${ke(`h00_${t}`, `cell${t}.x`, `cell${t}.y`)}
          ${ke(`h10_${t}`, `cell${t}.x + 1.0`, `cell${t}.y`)}
          ${ke(`h01_${t}`, `cell${t}.x`, `cell${t}.y + 1.0`)}
          ${ke(`h11_${t}`, `cell${t}.x + 1.0`, `cell${t}.y + 1.0`)}
...
v({
	type: "clouds",
	sampleCpu: (e, t) => Te(e, t),
	updateLive: (e, t) => e.applyLayerParams(t),
	wgsl: je,
	getTopologyKey: (e) => je.getTopologyKey(e)
});
//#endregion
...
function Le(e) {
	return e.map((e) => {
		let t = Me(e.layer.params);
		return {
			axis: A(Ie(e.layer.params.rotation)),
			layerId: e.layer.id,
...
function Re(e, t) {
	...
	n.axis.value.copy(Ie(t.params.rotation)), n.stops.forEach((e, t) => {
		...
		e.color.value.copy(Ne(n)), e.midpoint.value = n.midpoint, e.t.value = n.t;
	});
}
function ze(e) {
	let t = [];
...
function Ge(e) {
	return e.map((e) => ({
		...
		anchors: Array.from({ length: e.anchorCount }, (t, n) => {
			...
			return {
				color: A(P(r.color)),
				direction: A(We(r.x, r.y))
			};
		}),
		frequency: A(Math.max(1e-4, e.layer.params.frequency)),
		layerId: e.layer.id,
		mode: A(Ue(e.layer.params.mode)),
...
function Ke(e, t) {
	...
	n && (n.amplitude.value = y(t.params.amplitude, 0, .6), n.frequency.value = Math.max(1e-4, t.params.frequency), n.mode.value = Ue(t.params.mode), n.power.value = Math.max(1e-4, t.params.power), n.anchors.forEach((e, n) => {
		...
		e.color.value.copy(P(r.color)), e.direction.value.copy(We(r.x, r.y));
	}));
}
function qe(e) {
	let t = [];
...
function Je(e) {
    ...
    if (warpScale > 0.0) {
      let warpX = sin((direction.y * warpFrequency + 0.23) * ${F(Math.PI * 2)}) * cos((direction.z * warpFrequency + 0.41) * ${F(Math.PI * 2)});
      let warpY = cos((direction.z * warpFrequency + 0.17) * ${F(Math.PI * 2)}) * sin((direction.x * warpFrequency + 0.37) * ${F(Math.PI * 2)});
      let warpZ = sin((direction.x * warpFrequency - 0.31) * ${F(Math.PI * 2)}) * cos((direction.y * warpFrequency + 0.29) * ${F(Math.PI * 2)});
...
function G({ angularHeight: e, angularWidth: t, baseAngularHeight: n, baseAngularWidth: r, centerDirection: i, rotation: a = 0, upDirection: o = V }) {
...
var q = new T.DataTexture(new Uint8Array([
	0,
	0,
	0,
	0
]), 1, 1, T.RGBAFormat);
q.colorSpace = T.SRGBColorSpace, q.needsUpdate = !0;
//#endregion
//#region src/skybox/overlay.ts
var vt = .18, yt = .75, bt = 1.75, xt = 1e-4, St = .01;
//#endregion
...
function Tt(e) {
	return e.map((e) => {
		let t = wt(e.layer.params.placement);
		return {
...
function kt(e, t) {
	...
	return n <= 0 || r <= 0 ? "return vec4<f32>(0.0, 0.0, 0.0, 0.0);" : `
      ...
      let imageEdgeWidth = clamp(fwidth(imageEdgeDistance), 0.000001, ${F(St)});
      let imageHardInside = step(${F(xt)}, imageDenom) *
...
var Ft = {
	...
	type: "image",
	updateUniforms: (e, t) => Et(e, t.id, t.params.placement)
};
v({
	type: "image",
	sampleCpu: (e, t) => Ct(e, t),
	updateLive: (e, t) => e.applyImagePlacement(t.id, t.params.placement),
	wgsl: Ft,
	wgslEditorOverlay: !0,
	getTopologyKey: (e) => Ft.getTopologyKey(e)
});
//#endregion
...
function Y(e) {
	let t = e, n = zt(), r = Math.max(1e-4, typeof t?.baseAngularRadius == "number" ? t.baseAngularRadius : n.baseAngularRadius);
	return {
...
function Bt(e) {
	let t = W(e.centerDirection);
	return {
		x: Rt(Lt(Math.atan2(t[0], -t[2]))),
		y: Lt(Math.asin(J(t[1], -1, 1)))
	};
}
function Vt(e, t) {
	return {
...
function Jt(e, t) {
	return Math.max(Kt(e, t.hoveredLayerId), qt(e, t.selectedLayerId));
}
function Yt(e, t) {
	return e.map((e) => ({
		active: A(Jt(e.layer.id, t)),
		layerId: e.layer.id
	}));
}
function Xt(e, t) {
	e.forEach((e) => {
		e.active.value = Jt(e.layerId, t);
	});
}
function Zt(e, t) {
...
var Qt = j(`
  fn skyboxStudioApplyImageEditorRectOverlay(
    ...
    let edgeWidth = clamp(fwidth(edgeDistance), 0.000001, ${F(St)});
    ...
    let rectAlpha = rectCoverage * ${F(vt)};
...
`), $t = j(`
  fn skyboxStudioSpotEditorRectInfo(
    ...
    let spotEdgeWidth = clamp(fwidth(spotEdgeDistance), 0.000001, ${F(St)});
    let spotValid = step(${F(xt)}, spotDenom) *
...
function en(e, i) {
	...
	let ce = Math.max(s.haloInnerWidth, 1e-4), w = Math.max(s.haloOuterWidth, 1e-4), T = b - s.haloRadius, E = Math.exp(-ee(T / (T < 0 ? ce : w))), D = ae(d([
		1,
		1,
		1
	], g(y((b - (s.haloRadius - ce)) / (ce + w))), s.dispersion), ne), le = E * s.haloStrength * re;
	C = n(C, _(D, le)), C = n(C, _([
		1,
		1,
		1
	], Math.max(le - 1.2, 0) * .22));
	let ue = Math.abs(v.y), O = Math.abs(v.x), de = Math.exp(-ee((O - s.haloRadius) / Math.max(s.dogSpread, 1e-4))) * Math.exp(-ee(ue / Math.max(s.dogSpread * .72, 1e-4))), fe = a(s.haloRadius, s.haloRadius + Math.max(s.dogStretch, 1e-4), O) * (1 - a(s.haloRadius + Math.max(s.dogStretch, 1e-4), s.haloRadius + Math.max(s.dogStretch * 2.2, 1e-4), O)) * Math.exp(-ee(ue / Math.max(s.dogSpread * .9, 1e-4))), pe = ae(d([
		1,
		1,
		1
	], g(y((O - (s.haloRadius - s.dogSpread * 1.4)) / Math.max(s.dogSpread * 3.5, 1e-4))), s.dispersion), ne), k = (de + fe * .28) * s.dogStrength * re;
	C = n(C, _(pe, k)), C = n(C, _([
		1,
		1,
		1
	], Math.max(k - 1.1, 0) * .18));
...
function cn(e) {
	let t = `${e.parameterPrefix}Mode > 0.5`, n = sn(e);
	return `{
...
v({
	type: "spot",
	sampleCpu: (e, t) => en(e, t),
	updateLive: (e, t) => e.applyLayerParams(t),
	wgsl: ln,
	wgslEditorOverlay: !0,
	getTopologyKey: (e) => ln.getTopologyKey(e)
});
//#endregion
      ...
      ${e.parameterName}: vec4<f32>`).join(""),
		...
		createSampleNodes: ({ bindings: e, direction: t, imageTextures: n }) => {
			let r = n, i = /* @__PURE__ */ new Map(), a = Object.fromEntries(e.map((e) => {
				let n = mn({ direction: t }), a = k(fn(r, e.layer), n).setName(`starfieldTexture${e.index}`);
...
function gn(t, n, r = {}) {
	return n.filter((e) => e.enabled).reverse().reduce((n, i) => {
		let a = i.type === "group" ? [...gn(t, i.children, r), 1] : hn(t, i, r), o = y(a[3] * (i.opacity / 100));
		return e(n, [
			a[0],
			a[1],
			a[2]
...
function _n(e, t) {
	for (let n of e) if (n.type === "group") {
		if (n.id === t) return n;
		let e = _n(n.children, t);
		if (e) return e;
	}
	return null;
}
function vn(e, t, n = {}) {
	let r = N(e), i = n.targetGroupId ? _n(r.nodes, n.targetGroupId) : null;
	return gn(t, n.targetGroupId ? i ? [i] : [] : r.nodes, n);
}
//#endregion
//#region src/baking/bake.ts
var yn = 1024, bn = "0.1.0", xn = /* @__PURE__ */ new Map(), Sn = /* @__PURE__ */ new Map();
function Cn(e = {}) {
...
function Dn(e, t) {
	for (let n of e) {
		...
		if (n.id === t) return n;
		let e = Dn(n.children, t);
		if (e) return e;
	}
	return null;
}
function On(e, t, n, r, i) {
	let a = En(r ? Dn(e.nodes, r)?.children ?? [] : e.nodes);
	...
	return a.forEach((e) => {
		...
		let a = oe(e.params, t, n), s = Sn.get(a), c = s ?? b(e.params, t, n);
		s || Sn.set(a, c), o.set(e.id, c);
	}), o;
}
function kn(e, t = {}) {
	let n = N(e), r = Cn(t), i = r.cache ? wn(n, r) : null;
	if (i) {
		let e = xn.get(i);
		if (e) return {
			...e,
	...
	let { height: a, targetGroupId: o, width: s } = r, c = On(n, s, a, o, t.starfieldBakes), l = new Uint8ClampedArray(s * a * 4);
	for (let e = 0; e < a; e += 1) {
		let t = (e + .5) / a;
		for (let r = 0; r < s; r += 1) {
			let [i, d, f] = se(vn(n, u((r + .5) / s, t), {
				sampleHeight: a,
				starfieldBakes: c,
				targetGroupId: o
			})), p = (e * s + r) * 4;
...
function Mn(e) {
	return {
		blendMode: jn(e.blendMode),
		opacity: y(e.opacity / 100)
	};
}
function Nn(e) {
	return `select(1.055 * pow(${e}, ${I(1 / 2.4)}) - ${I(.055)}, ${e} * 12.92, ${e} <= ${I(.0031308)})`;
}
function Pn(e) {
	return `select(pow((${e} + ${I(.055)}) / ${I(1.055)}, ${I(2.4)}), ${e} / 12.92, ${e} <= ${I(.04045)})`;
}
function Fn(e) {
	let t = I(1), n = I(.5), r = I(0), i = "blendSource", a = "blendBackdrop";
	switch (e) {
...
function In() {
	return `let softLightD = ${R("blendBackdrop <= vec3<f32>(0.25)", "((16.0 * blendBackdrop - vec3<f32>(12.0)) * blendBackdrop + vec3<f32>(4.0)) * blendBackdrop", "sqrt(blendBackdrop)")};`;
}
function Ln(e, t) {
	let n = jn(t);
	return `${e} >= ${F(n - .5)} && ${e} < ${F(n + .5)}`;
}
function Rn(e) {
	let t = [
		"darken",
		...
		"difference",
		"exclusion"
	].map((t, n) => `${n === 0 ? "if" : "else if"} (${Ln(e, t)}) {
            blendedSrgb = ${Fn(t)};
          }`).join("\n");
	return `let blendSourceLinear = clamp(effectColor.rgb, vec3<f32>(0.0), vec3<f32>(1.0));
        ${L("blendedColor", "vec3<f32>", "blendSourceLinear")}
        if (${e} >= ${F(.5)}) {
          let blendBackdropLinear = clamp(composedColor, vec3<f32>(0.0), vec3<f32>(1.0));
          let blendBackdrop = ${Nn("blendBackdropLinear")};
          let blendSource = ${Nn("blendSourceLinear")};
          ${In()}
          ${L("blendedSrgb", "vec3<f32>", "blendSource")}
          ${t}
          let blendedSrgbClamped = clamp(blendedSrgb, vec3<f32>(0.0), vec3<f32>(1.0));
          blendedColor = ${Pn("blendedSrgbClamped")};
        }`;
}
function zn(e, t, n, r = 0) {
	return An(e).map((e, i) => {
		let a = e.type === "group" ? `effectColor = vec4<f32>(groupColor${r}_${i}, 1.0);` : Vn(e, n), o = `groupColor${r}_${i}`, s = t.get(e.id), c = s ? `${s.parameterPrefix}Opacity` : F(e.opacity / 100), l = s ? `${s.parameterPrefix}BlendMode` : F(jn(e.blendMode));
		return `{
        ${e.type === "group" ? `${L(o, "vec3<f32>", "vec3<f32>(0.0)")}
        {
          ${L("previousComposedColor", "vec3<f32>", "composedColor")}
          composedColor = vec3<f32>(0.0);
          ${zn(e.children, t, n, r + 1)}
          ${o} = composedColor;
...
function Hn(e) {
	return e.type === "group" ? e.children.some(Hn) : e.type === "starfield";
}
function Un(e) {
	let t = An(e), n = -1;
...
function ar(e, t, n, r, i) {
	let a = Zn(e.nodes), o = Gn(a), s = er(e, t, n, r, i), c = tr(s, "image"), l = c?.uniforms ?? [], u = c?.samples, d = tr(s, "starfield")?.samples;
	return {
		colorNode: ir(e, s, a)({
			direction: t,
			...s.sampleParameters,
...
function sr(e, t, n, r, i, a) {
	...
	let { colorNode: c, compositionUniforms: l, imageSamples: u, imageUniforms: d, layerRuntime: f, starfieldSamples: p } = ar(e, or(), n, r, i), m = a ? x().flatMap((e) => {
		let n = f.adapters.get(e.type);
		if (!e.wgslEditorOverlay || !n) return [];
		let r = n.bindings;
		return [{
			bindings: r,
			editorUniforms: Yt(r, t)
		}];
	}) : [], h = c;
	return m.forEach(({ bindings: e, editorUniforms: t }) => {
		...
		m.forEach(({ editorUniforms: t }) => Xt(t, e));
	}), o.userData.webGpuLayerRuntime = f, o.userData.applyLayerParams = (e) => rr(f, e), Yn(o, (e) => qn(l, e)), Xn(o, (e) => Jn(l, e)), Dt(o, (e, t) => Et(d, e, t)), o.userData.applyImageTextures = (e) => Nt(u?.sampleData ?? /* @__PURE__ */ new Map(), e), o.userData.applyStarfieldTextures = (e) => pn(p?.sampleData ?? /* @__PURE__ */ new Map(), e), o.userData.debugImageTextureSlots = f.textureSlotsByLayerId, o;
}
function cr(e, t) {
	let n = Wn(e.nodes, t);
	return j(`
...
function lr(e, t, n, r) {
	let i = new E();
	i.side = T.BackSide, i.depthTest = !1, i.depthWrite = !1, i.vertexNode = D(() => {
		let e = O;
		return e.z.assign(e.w), e;
	})();
	let a = or(), o = er(e, a, t, n, r), s = tr(o, "image"), c = s?.samples, l = s?.uniforms ?? [], u = tr(o, "starfield")?.samples;
	return i.colorNode = cr(e, o)({
		direction: a,
		...o.sampleParameters
	}), i.userData.applyLayerParams = (e) => rr(o, e), Dt(i, (e, t) => Et(l, e, t)), i.userData.applyImageTextures = (e) => Nt(c?.sampleData ?? /* @__PURE__ */ new Map(), e), i.userData.applyStarfieldTextures = (e) => pn(u?.sampleData ?? /* @__PURE__ */ new Map(), e), i;
}
...
function fr(e, t, n, r = {}) {
	...
	let a = fe.xy.mul(.5).add(.5), { colorNode: o } = ar(e, de(dr({ uv: r.flipY ? me(a.x, a.y.oneMinus()) : a })), t, n, /* @__PURE__ */ new Map());
...
function pr(e) {
	...
	return t.side = T.BackSide, t.depthTest = !1, t.depthWrite = !1, t.vertexNode = n, t.colorNode = k(e, ur({ direction: r })), t;
}
function mr(e, t) {
...
function hr(e, t = {}) {
	let n = kn(e, t), r = mr(n.width, n.height), i = r.getContext("2d");
...
var yr = { starsOmitted: !0 }, br = {
	...
	#w = new T.Vector2();
	constructor() {
		super(_e(M), sr(br, Gt, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), !1)), this.frustumCulled = !1, this.renderOrder = -1, this.onBeforeRender = ((e, t, n) => {
	...
	applyGeometry(e) {
		...
		let n = this.geometry;
		this.#r = t, this.geometry = _e(t), n.dispose();
	}
	disposeOwnedTexture() {
	...
	disposeStarfieldTextures() {
		this.#m.forEach((e) => {
			clearTimeout(e);
		}), this.#m.clear(), this.#g.forEach((e) => dn(e)), this.#g.clear(), this.#h.clear(), this.#f?.dispose(), this.#f = null;
	}
	...
	syncStarfieldGlint(e, t) {
		let n = this.#f;
		if (!n?.createGlints || _r(this.#u) !== "live-webgpu") {
	...
	coverageActive() {
		return _r(this.#u) === "live-webgpu" && this.#_.size > 0 && Un(this.#s.nodes);
	}
	disposeCoverage() {
	...
	syncCoverage(e) {
		...
		(!this.#x || this.#C !== e) && (this.#x?.dispose(), this.#b && this.#v.remove(this.#b), this.#x = lr(this.#s, this.#a, this.#g, /* @__PURE__ */ new Map()), this.#x.userData.applyImageTextures?.(this.#a), this.#i.forEach((e, t) => {
			this.#x?.userData.applyImageLayerPlacement?.(t, e);
		}), this.#y ||= _e(M), this.#b = new T.Mesh(this.#y, this.#x), this.#b.frustumCulled = !1, this.#v.add(this.#b), this.#S ||= new T.RenderTarget(1, 1, { depthBuffer: !1 }), this.#C = e);
	...
	scheduleStarfieldTextureBake(e, t) {
		...
		let n = this.#f?.createBakeKey(t, void 0, null, yr) ?? "";
		...
		let i = setTimeout(() => {
			...
			if (t?.type !== "starfield") return;
			let r = this.#f?.createBakeKey(t.params, void 0, null, yr) ?? "";
			if (r !== n) {
				this.scheduleStarfieldTextureBake(e, t.params);
				return;
			}
			if (!this.#f && this.#d && (this.#f = C(this.#d)), !this.#f?.canBake()) return;
			let i = this.#f.bakeTexture(t.params, r, void 0, null, yr), a = this.#g.get(e);
			a && a !== i && dn(a), this.#g.set(e, i), this.#h.set(e, r), a ? this.refreshStarfieldTextureBindings() : (this.#c = null, this.setManifest(this.#s)), this.dispatchEvent({ type: "starfieldtexturechange" });
		}, 150);
		this.#m.set(e, i);
	}
	...
	setManifest(e) {
		...
		let n = _r(this.#u), r = vr(this.#s, n, this.#n);
		if (this.#c === r && n === "live-webgpu") return this.applyLiveManifestUniformUpdates(), this.syncCoverage(r), this;
		if (n === "live-webgpu") this.replaceMaterial(sr(this.#s, this.#t, this.#a, this.#g, /* @__PURE__ */ new Map(), this.#n));
		else {
			let e = hr(this.#s, this.#e);
			this.replaceMaterial(gr(e), e);
		}
		return this.#c = r, this.syncCoverage(r), this;
	}
	setBakedTexture(e) {
		return this.replaceMaterial(gr(e)), this.#c = null, this;
	}
	invalidateBakeCache() {
		return Tn(), this;
	}
...
var wr = class {
	...
	bakeRenderTarget(e, t) {
		let n = Math.max(1, Math.floor(t.width)), r = Math.max(1, Math.floor(t.height)), i = fr(N(e), t.imageTextures ?? /* @__PURE__ */ new Map(), t.starfieldTextures ?? /* @__PURE__ */ new Map(), { flipY: t.flipY }), a = Cr(n, r, !!t.hdr, !!t.float), o = new T.Mesh(this.#r, i);
		o.frustumCulled = !1;
...
var Q = class {
	...
	async load(e, t) {
		...
		for (let e of n) try {
			return Dr(await this.#e.loadAsync(e));
		} catch (n) {
...
var Nr = 101010256, Pr = 33639248, Fr = 67324752, Ir = 22, Lr = 65535;
function Rr(e) {
	let t = Math.max(0, e.byteLength - Ir - Lr);
	for (let n = e.byteLength - Ir; n >= t; --n) if (e.getUint32(n, !0) === Nr) return n;
	return -1;
}
async function zr(e) {
	let t = new DataView(e.buffer, e.byteOffset, e.byteLength), n = Rr(t);
	...
	for (let n = 0; n < r; n += 1) {
		...
		if (t.getUint32(u, !0) !== Fr) throw Error(`Invalid zip bundle: bad local header for "${d}".`);
		let f = t.getUint16(u + 26, !0), p = t.getUint16(u + 28, !0), m = u + 30 + f + p, h = e.subarray(m, m + r);
		if (n === 0) o.push(Promise.resolve([d, h]));
		else if (n === 8) o.push(Mr(h).then((e) => [d, e]));
...
async function Br(e, t = {}) {
	let n = t.toAssetUrl ?? Ar, r = await zr(await jr(e)), i = r[$];
...
async function Ur(e) {
	...
	for (let t of Or(n)) t.params.src && r.set(t.params.src, await Hr(e, t.params.src));
	return {
		manifest: n,
...
async function Kr(e, t = {}) {
	let n = t.loader ?? Gr(), r = Or(e.manifest).filter((e) => e.enabled && e.params.src), i = r.map((t) => ({
		id: t.id,
...
async function Jr(e, t = {}) {
	let { onProgress: n, ...r } = t, i = !qr(e), a = qr(e) ? e : await Br(e, r), o = Gr(), s = await Kr(a, {
		loader: o,
		onProgress: n
	});
	return {
...
//#endregion
export { yn as DEFAULT_BAKE_WIDTH, It as DEFAULT_SPOT_BASE_ANGULAR_RADIUS, $e as IMAGE_PLACEMENT_ELEVATION_LIMIT, Er as Loader, Z as LoaderAssetError, xr as Skybox, wr as SkyboxGpuBakeService, Q as TextureLoaderExtension, kn as bakeSkyboxImageData, te as blendChannel, y as clamp, Or as collectImageLayers, m as compositeBlendChannel, e as compositeOver, Dr as configureSkyboxImageTexture, G as createAngularDecalPlacement, wn as createBakeCacheKey, hr as createBakedSkyboxTexture, zt as createDefaultSpotParams, lt as createImagePlacementTangents, _e as createSkyboxGeometry, Tr as createSkyboxGpuBakeService, ye as createSkyboxWireGeometry, dt as directionFromPosition, vn as evaluateSkyboxDirection, c as getLayerRuntimeAdapter, x as getLayerRuntimeAdapters, Tn as invalidateBakeCache, s as isRegisteredLayerType, ce as linearChannelToSrgb, se as linearRgbToSrgbBytes, Ur as loadBundleFromDirectory, Vr as loadBundleFromUrl, Br as loadBundleFromZip, Jr as loadSkyboxBundle, Kr as loadSkyboxImageTextures, N as migrateManifestToV2, K as normalizeImagePlacement, Y as normalizeSpotParams, W as normalizeVector, S as parseHexColor, ft as placementFromPosition, gt as placementFromRotation, mt as placementFromScale, ut as positionFromPlacement, Bt as positionFromSpot, _t as projectDirectionToImageUv, Ht as radiusScaleFromSpot, v as registerLayerRuntimeAdapter, Wr as rehydrateImagePixels, Cn as resolveBakeOptions, ht as rotationFromPlacement, pt as scaleFromPlacement, Wt as spotContainsDirection, Vt as spotFromPosition, Ut as spotFromRadiusScale, re as srgbChannelToLinear };
