import { B as e, C as t, E as n, P as r, S as i, T as a, _ as o, a as s, b as c, c as l, d as u, f as d, g as f, h as p, i as m, it as h, l as g, m as _, n as v, o as y, p as b, q as x, r as S, s as C, u as w, v as T, w as E, x as D, y as O } from "./starfield-bake-registry-D5mi0bgU.js";
import * as k from "three";
import { Fn as A, If as j, Loop as M, PI as N, acos as ee, attribute as P, cameraProjectionMatrix as te, cameraViewMatrix as ne, clamp as F, cos as re, dot as ie, exp as I, float as L, floor as ae, int as oe, length as se, max as R, min as ce, mix as z, mod as le, modelWorldMatrix as ue, mx_fractal_noise_float as de, normalize as fe, positionGeometry as pe, pow as B, screenSize as me, screenUV as he, select as V, sin as ge, smoothstep as H, step as U, texture as _e, uniform as ve, uniformArray as ye, uniformTexture as be, uv as xe, varyingProperty as W, vec2 as G, vec3 as K, vec4 as q } from "three/tsl";
import { MeshBasicNodeMaterial as Se } from "three/webgpu";
//#region src/baking/starfield-gpu-bake.ts
Math.PI * 2;
var Ce = 8, J = g / 2, we = 1.75, Te = 3.25, Y = 1, Ee = 1.5, De = 8, Oe = .1, ke = 5, Ae = 12, je = .35, Me = .25;
function Ne(e, t) {
...
function Ie(e) {
	...
	return typeof n?.limits?.maxTextureDimension2D == "number" ? n.limits.maxTextureDimension2D : r ? Number(r.getParameter(r.MAX_TEXTURE_SIZE)) : g;
}
function X(e, t) {
	let n = e[t];
	if (n?.isUniformNode) return n;
	let r = ve(Number(n?.value ?? 0));
	return e[t] = r, r;
}
function Z(e, t) {
...
function Re(e) {
	let t = e.x.sub(.5).mul(N).mul(2), n = e.y.mul(N), r = ge(n);
	return fe(K(r.mul(ge(t)), re(n), r.mul(re(t))));
}
function ze(e) {
	let t = le(e.y, 2), n = U(1, t);
...
function Q(e, t, n, r) {
	...
	return M(8, ({ i: e }) => {
		j(L(e).lessThan(i), () => {
			let e = de(s, oe(1), a, o).mul(.5).add(.5);
...
function Ye(e, t) {
	...
	X(s, "uNebulaExposure");
	let D = X(s, "uNebulaStrength"), O = Le(s, "uCloudShadow"), N = Le(s, "uCloudHighlight"), ee = Le(s, "uCloudCore"), P = ye(i, "vec3"), te = ye(a, "vec3"), ne = new Se({
		depthTest: !1,
		depthWrite: !1
	});
	return ne.uniforms = s, ne.colorNode = A(() => {
		let e = pe.xy.mul(.5).add(.5), t = Be(c.add(e.mul(l))), n = F(S, 1, 8), r = t.mul(R(h, .001)).add(K(g, g.mul(.37), g.mul(-.21))), i = K(Q(r, n, 2.02, .52), Q(r.add(K(5.2, 1.3, 7.1)), n, 2.03, .5), Q(r.add(K(9.1, 8.4, 2.8)), n, 2.01, .51)).mul(2).sub(1), a = fe(t.add(i.mul(R(m, 0)))), o = K(0).toVar(), s = L(0).toVar();
		M(Ce, ({ i: e }) => {
			j(L(e).lessThan(u), () => {
				let t = fe(P.element(e)), n = te.element(e), r = L(1).sub(ie(a, t)), i = L(1).div(B(r.add(1e-4), R(f, 1e-4))), c = I(r.mul(r).negate().div(R(1e-4, L(2).mul(p).mul(p)))), l = V(d.lessThan(.5), i, c);
				o.addAssign(n.mul(l)), s.addAssign(l);
			});
		}), o.assign(o.div(R(s, 1e-4)));
		let k = K(g.mul(13.17), g.mul(-7.31), g.mul(5.19)), A = t.mul(R(x, .001)).add(k), ne = K(Q(A, n, 2.02, .5), Q(A.add(K(5.2, 1.3, 2.8)), n, 2.02, .5), Q(A.add(K(2.1, 4.7, 9.2)), n, 2.02, .5)), re = F(Q(A.add(ne.mul(3)), n, 2.02, .5), 0, 1), ae = B(F(H(_, _.add(R(y, .001)), re), 0, 1), R(b, .05)), oe = B(F(R(R(o.r, o.g), o.b).mul(R(E, 0)), 0, 1), R(w, .001)), se = B(R(z(z(O, o.mul(N).mul(R(E, 0)), oe), ee, F(ae.mul(.4), 0, 1)).add(o.mul(oe).mul(ae.oneMinus()).mul(R(T, 0)).mul(R(E, 0))).mul(R(v, 0)), K(0)), K(.92)), ce = F(ae.mul(C), 0, 1);
		return q(R(K(.004, .005, .011).add(se.mul(ce).mul(R(D, 0))), K(0)), 1);
	})(), ne;
}
function Xe(e, t, n) {
	let r = b(e.stars, t, n, { includeSeamCopies: !0 }), i = [], a = [], o = [], s = [], c = [];
	r.forEach((e) => {
...
function $e(e) {
	let t = o(e.clip), n = d(e.stars, t, J, { includeSeamCopies: !1 }), r = [], i = [], s = [];
	n.forEach((t) => {
		a([
			t.x,
			t.y,
			t.z
		], e.clip) && (r.push(t.x, t.y, t.z), i.push(t.rSize, t.rBright, t.rGlare, t.rColor), s.push(t.rSizeGate));
	});
	let c = new k.InstancedBufferGeometry();
	return c.setAttribute("position", new k.BufferAttribute(Pe, 3)), c.setAttribute("iDirection", new k.InstancedBufferAttribute(new Float32Array(r), 3)), c.setAttribute("iRandoms", new k.InstancedBufferAttribute(new Float32Array(i), 4)), c.setAttribute("iSizeGate", new k.InstancedBufferAttribute(new Float32Array(s), 1)), c.instanceCount = s.length, c.boundingSphere = new k.Sphere(new k.Vector3(0, 0, 0), Infinity), c;
}
function et(e) {
	let t = e.stars;
	return {
...
function tt(e) {
	...
	}, r = X(n, "uCoverageEnabled"), i = be(dt());
	n.uCoverageTexture = i;
	let a = X(n, "uStarSize"), o = X(n, "uSizeVar"), s = X(n, "uLargeStarRarity"), c = X(n, "uBright"), l = X(n, "uBrightVar"), u = X(n, "uGlareSize"), d = X(n, "uGlareStr"), f = X(n, "uGlareVar"), p = X(n, "uColorVar"), m = X(n, "uRenderHeight"), h = W("vec2", "vStarGlintLocalPx"), g = W("vec4", "vStarGlintRandoms"), _ = W("float", "vStarGlintSizeGate"), v = new Se({
		blending: k.AdditiveBlending,
		depthTest: !1,
		depthWrite: !1,
		toneMapped: !1,
	...
	return v.uniforms = n, v.vertexNode = A(() => {
		let e = P("iDirection", "vec3"), t = P("iRandoms", "vec4"), n = P("iSizeGate", "float"), r = Je(t.x, n, s, o), i = a.mul(r), c = H(Y, Ee, i).oneMinus(), l = R(R(i, z(L(we), L(.5), c)).mul(.45), L(.5)), f = u.mul(z(1, r, o)), p = R(R(l, R(R(i.add(f), L(Te)).mul(.36), L(.5)).mul(U(1e-6, u)).mul(U(1e-6, d))), L(.5)).mul(De), v = pe.xy, y = me, b = ue.mul(q(fe(e), 0)).xyz, x = ne.mul(q(b, 0)).xyz, S = te.mul(q(x, 1)), C = y.y.div(R(m, 1)), w = q(v.mul(p).mul(C).div(y.mul(.5)).mul(S.w), 0, 0);
		return h.assign(v.mul(p)), g.assign(t), _.assign(n), V(S.w.greaterThan(0), S.add(w), q(2, 2, 2, 1));
	})(), v.colorNode = A(() => {
		let e = se(h), t = qe(g.x, _, s), n = Je(g.x, _, s, o), m = a.mul(n), v = H(Y * .75, Y, m).oneMinus(), y = H(Ee, 1.75, m), b = R(m, L(.1)), x = z(1, R(.08, H(0, Y, m)), v), S = R(b.mul(.45), L(.5)), C = I(e.mul(e).negate().div(R(S.mul(S).mul(2), 1e-10))).mul(x), w = u.mul(z(1, n, o)), T = R(R(m.add(w), L(.1)).mul(.36), L(.5)), E = I(e.mul(e).negate().div(R(T.mul(T).mul(2), 1e-10))).mul(y).mul(U(1e-6, u)).mul(U(1e-6, d)), D = z(g.y, R(g.y, t), o.mul(je)), O = z(g.z, R(g.z, t), o.mul(Me)), k = d.mul(z(1, B(O, 8), f)), A = c.mul(z(1, B(D, 3).mul(3), l)), j = Ke(z(.5, g.w, p)).mul(C.add(E.mul(k))).mul(A), M = _e(i, he).rgb;
		return q(j.mul(z(K(1), M, r)), 1);
	})(), {
		material: v,
		uniforms: n
	};
...
function rt(e, t, n, r, i, a) {
	...
	}, s = be(e), c = Z(o, "uSourceSize"), l = Z(o, "uTargetSize"), u = X(o, "uSourcePerTarget"), d = X(o, "uExposure"), f = new Se({
		depthTest: !1,
		depthWrite: !1
	});
	return f.uniforms = {
		...
		let e = ae(xe().mul(l)), t = ae(u.add(.5)), n = q(0).toVar(), r = L(0).toVar();
		M(8, ({ i }) => {
			M(8, ({ i: a }) => {
				j(L(a).lessThan(t).and(L(i).lessThan(t)), () => {
					let t = e.mul(u).add(G(L(a), L(i))).add(.5);
					n.addAssign(_e(s, t.div(c))), r.addAssign(1);
				});
			});
		});
...
function it(e, t, n, r) {
	...
	}, a = be(e), o = be(t), s = Z(i, "uContentUvMin"), c = Z(i, "uContentUvSize"), l = Z(i, "uStorageUvMin"), u = Z(i, "uStorageUvSize"), d = X(i, "uHasLeftNeighbor"), f = X(i, "uHasRightNeighbor"), p = X(i, "uHasTopNeighbor"), m = X(i, "uHasBottomNeighbor"), h = X(i, "uNebulaExposure"), g = new Se({
		depthTest: !1,
		depthWrite: !1,
		transparent: !0
	}), _ = +(n.uvSize.x >= .999), v = .28;
	return g.blending = k.CustomBlending, g.blendEquation = k.AddEquation, g.blendSrc = k.OneFactor, g.blendDst = k.OneFactor, g.blendEquationAlpha = k.AddEquation, g.blendSrcAlpha = k.OneFactor, g.blendDstAlpha = k.OneMinusSrcAlphaFactor, i.uNebulaTexture = a, i.uStarTexture = o, g.uniforms = i, g.colorNode = A(() => {
		let e = pe.xy.mul(.5).add(.5), t = G(e.x, L(1).sub(e.y)), n = R(L(1).sub(H(0, v, t.y)), L(1).sub(H(0, v, L(1).sub(t.y)))).mul(_), r = We(t, l, u), i = F(r, 0, 1), g = Ge(r), y = G(Ue(t.x, s.x, c.x).div(c.x), t.y.sub(s.y).div(c.y)), b = R(u.sub(c).div(c.mul(2)), G(0)), x = R(b, G(1e-6)), S = V(d.greaterThan(.5), H(x.x.negate(), x.x, y.x), 1), C = V(f.greaterThan(.5), L(1).sub(H(L(1).sub(x.x), L(1).add(x.x), y.x)), 1), w = V(b.x.lessThanEqual(0), 1, S.mul(C)), T = V(p.greaterThan(.5), H(x.y.negate(), x.y, y.y), 1), E = V(m.greaterThan(.5), L(1).sub(H(L(1).sub(x.y), L(1).add(x.y), y.y)), 1), D = V(b.y.lessThanEqual(0), 1, T.mul(E)), O = F(w.mul(D).mul(g), 0, 1), k = _e(a, i).rgb, A = K(0).toVar(), j = L(0).toVar();
		M(32, ({ i: e }) => {
			let n = We(G(L(e).add(.5).div(32), t.y), l, u), r = F(n, 0, 1), i = Ge(n);
			A.addAssign(_e(a, r).rgb.mul(i)), j.addAssign(i);
		});
		let N = z(k, A.div(R(j, 1)), n), ee = _e(o, i);
		return q(K(1).sub(I(N.mul(R(h, .001)).negate())).add(ee.rgb), 1).mul(O);
...
var gt = class {
	...
	createBakeKey(e, t, n, r) {
		let i = T(e), a = f(i.quality), o = mt(t), s = _(i, o, Math.floor(o / 2), {
	...
	previewWidthFor(e) {
		return Math.max(1, Math.min(g, this.#n));
	}
	...
	#s(e, t, n, r) {
		let i = T(e), a = f(i.quality), o = mt(n), s = Math.floor(o / 2), c = t ?? this.createBakeKey(i, o, r), l = this.#t.get(c);
		if (l) return l;
		let u = p({
			budgetBytes: a.budgetBytes,
			clip: i.clip,
			...
			maxTextureSize: this.#n,
			width: o
		}), d = this.#r.getRenderTarget(), m = this.#r.autoClear, h = Object.assign(new k.Color(), { a: 1 }), g = this.#r.getClearAlpha(), _ = [], v = [];
		this.#r.getClearColor(h), this.#r.autoClear = !0, this.#r.setClearColor(0, 0), u.descriptors.forEach((e) => {
		...
		}), this.#r.setRenderTarget(d), this.#r.autoClear = m, this.#r.setClearColor(h, g);
		let y = {
			key: c,
			patches: v,
			targets: _
	...
	#c(e, t, n, r, i) {
		let a = T(e), o = f(a.quality), s = mt(n), c = Math.floor(s / 2), l = ht(s, this.#n), u = Math.floor(l / 2), d = i?.starsOmitted ?? !1, m = t ?? this.createBakeKey(a, s, r, i), h = this.#e.get(m);
		if (h && h.target.width === l && h.target.height === u) return h;
		...
		}), this.#r.setRenderTarget(v), this.#r.autoClear = y, this.#r.setClearColor(b, x), g.texture.userData.starfieldRenderTarget = g, this.#e.get(m)?.target.dispose(), this.#e.set(m, {
			key: m,
			target: g,
			texture: g.texture
		}), {
...
function vt(t, n, i, a = {}) {
	...
	if (o) {
		let t = E(n), i = (t.u % 1 + 1) % 1 * o.width - .5, a = x(t.v, 0, 1) * o.height - .5, s = Math.floor(i), c = Math.floor(a), l = s + 1, u = c + 1, d = i - s, f = a - c;
		return r(r(e(o, s, c), e(o, l, c), d), r(e(o, s, u), e(o, l, u), d), f);
	}
...
//#endregion
export { S as DEFAULT_STARFIELD_CLIP, m as DEFAULT_STARFIELD_NEBULA, s as DEFAULT_STARFIELD_NEBULA_FIELD, y as DEFAULT_STARFIELD_PARAMS, C as DEFAULT_STARFIELD_QUALITY, l as DEFAULT_STARFIELD_STARS, g as STARFIELD_PREVIEW_BAKE_WIDTH, w as STARFIELD_QUALITY_PRESETS, gt as StarfieldGpuBakeService, u as bakeStarfieldImageData, d as createStarCatalogForCoverage, b as createStarCatalogForDescriptor, _ as createStarfieldBakeCacheKey, nt as createStarfieldGlints, _t as createStarfieldGpuBakeService, p as createStarfieldPatchLayout, f as getStarfieldQualityPreset, o as normalizeStarfieldCoverage, T as normalizeStarfieldParams, O as normalizeStarfieldQuality, c as qFromV, D as sampleStarfieldLayer, i as sourceDirectionFromUv, t as sourceFoldEquirectUv, E as sourceUvFromDirection, a as starfieldClipContainsDirection, n as starfieldFieldGradientToSourceField, Qe as starfieldGlintGeometryKey };
